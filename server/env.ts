...
export function validateEnv(): EnvConfig {
  ...
  // Production Guard: Prevent startup if critical variables are missing in production mode
  if (process.env.NODE_ENV === 'production') {
    ...
    if (!computedAppUrl || computedAppUrl.includes('localhost')) missingVars.push('APP_URL (must not default to localhost in production)');
